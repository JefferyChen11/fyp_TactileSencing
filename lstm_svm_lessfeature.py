...
# extra _200 training set: sample per kept class
NPZ_FILE_EXTRA_200     = "data_rebuild_merge_calib_crop_200.npz"
EXTRA_200_PER_CLASS    = 40         # number sampled per kept class from _200
EXTRA_200_RANDOM_STATE = 42

# -- avoid confusing with the original 5×6 version, rename model files --
SAVE_ENCODER  = "lstm_encoder_4x4.pt"
SAVE_SVM      = "lstm_svm_model_4x4.joblib"
...
# ========= default classes & mapping (used if NPZ doesn't contain 'classes') =========
CLASS_NAMES = [
...
# -- kept textures / softness --
KEEP_TEXTURES = ['smooth', 'strawberry', 'bigberry']  # discard: citrus, rough, smallberry
KEEP_SOFTNESS = ['ds20', 'ef50', 'ef30']              # discard: ds30, ef10

# for plotting use (will be rebuilt after loading)
...
class AugSeqDataset(Dataset):
    ...
    Training augmentations (applied in sequence):
      ...
      6) additive Gaussian noise (sigma=0.01)
    """
...
def plot_aggregated(y_true, y_pred, title_prefix="Cross-Validation Aggregated Results (Attn LSTM → SVM)"):
    ...
    # dynamic mapping: class id -> texture/softness indices
...
# ========= class-subset utilities: filter + remap =========
def build_allowed_classes(classes, keep_textures, keep_softness):
    """Filter classes (list of strings) to those matching keep_textures AND keep_softness.
       Return allowed class names (in original order) and a name->new-index mapping.
    """
    allowed = [name for name in classes
               if (name.split('_')[0] in keep_textures) and (name.split('_')[1] in keep_softness)]
    if len(allowed) == 0:
        raise RuntimeError("Allowed class set is empty: check KEEP_TEXTURES / KEEP_SOFTNESS against data classes.")
    name_to_new = {name: i for i, name in enumerate(allowed)}
    return allowed, name_to_new

def filter_dataset_to_allowed(X, y, classes, allowed_names, name_to_new):
    """Keep only samples whose class name is in allowed_names, and remap labels to [0..len(allowed)-1]."""
...
def load_and_align_extra_200(npz_path, allowed_names, name_to_new, per_class=20, seed=RANDOM_STATE):
    """From the _200 dataset, sample per_class examples per allowed class and map them to the new label space."""
    if not os.path.exists(npz_path):
        print(f"[INFO] Extra dataset not found: {npz_path}, skipping.")
        return None, None
    d = np.load(npz_path, allow_pickle=True)
    X2 = d["X"]; y2 = d["y"].astype(int)
    if 'classes' not in d.files:
        raise RuntimeError("Extra _200 dataset does not contain 'classes'. Please regenerate using the new data processing script.")
    classes2 = [str(x) for x in d['classes'].tolist()]

    rng = np.random.default_rng(seed)
    X2_sel_list, y2_sel_list = [], []
    for name in allowed_names:
        if name not in classes2:
            print(f"[WARN] _200 missing class {name}, skipping extra samples for this class.")
            continue
        c_idx = classes2.index(name)
        idx = np.where(y2 == c_idx)[0]
        if len(idx) == 0:
            print(f"[WARN] _200 class {name} has no samples, skipping.")
            continue
        take = min(per_class, len(idx))
        sel = rng.choice(idx, size=take, replace=False)
        X2_sel_list.append(X2[sel])
        y2_sel_list.append( np.full((take,), name_to_new[name], dtype=int) )

    if len(X2_sel_list) == 0:
        print("[WARN] Sampling from _200 returned an empty selection.")
        return None, None

    X2_sel = np.concatenate(X2_sel_list, axis=0)
    y2_sel = np.concatenate(y2_sel_list, axis=0)
    print(f"[INFO] Sampled {len(y2_sel)} examples from _200 dataset ({per_class} per class, limited to allowed classes).")
...
def main():
    ...
    if 'classes' in data_main.files:
        classes_main = [str(x) for x in data_main['classes'].tolist()]
    else:
        classes_main = CLASS_NAMES
        print("[WARN] main NPZ does not include 'classes'; using built-in CLASS_NAMES.")

    # 2) build allowed classes and filter + remap main training set
    ...
    X_main, y_main, mask_main = filter_dataset_to_allowed(X_main, y_main, classes_main, allowed_names, name_to_new)
    print(f"[INFO] After filtering, main training set: {X_main.shape[0]} samples / {len(allowed_names)} classes")

    # 3) load and merge _200 data (sample per allowed class)
    X2, y2 = load_and_align_extra_200(
        NPZ_FILE_EXTRA_200, allowed_names=allowed_names, name_to_new=name_to_new,
    ...
    print(f"[INFO] Total training samples: {len(y_all)} (main {len(y_main)} + extra {extra_cnt}), num_classes={len(CLASS_NAMES)}")

    # 4) shape and class checks
    assert X_all.ndim==4 and X_all.shape[1:]==(T,C,D), f"Expect X.shape[1:]==({T},{C},{D}), got {X_all.shape[1:]}"
    num_classes = len(CLASS_NAMES)

    # 5) normalize and reshape to sequences
